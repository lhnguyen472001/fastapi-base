...
class BaseSQLAlchemyRepository(Generic[SQLAlchemyModelT]):
    ...
    def __init__(self, *, statement: Select[tuple[SQLAlchemyModelT]] | None = None, **kwargs: Any) -> None:
        ...
        self.kwargs = kwargs
        # Composed collaborator introduced by the 3.1 Phase-A split. The
        # ``apply_filter`` / ``apply_order_by`` / ``filter_select_by_kwargs``
        # methods delegate here so query-composition logic can be tested
        # and reused without instantiating a repository.
