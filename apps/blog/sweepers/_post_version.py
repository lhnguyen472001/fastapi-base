...
Shape mirrors the autosave sweeper: every uvicorn worker spawns the
same forever loop, all workers race for a single Redis leader lock (a
distinct key from the autosave one so the two sweepers do not block
each other), and only the holder actually queries / deletes.
