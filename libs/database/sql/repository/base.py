...
from sqlalchemy.sql import ColumnElement, Delete, Select, Update, delete, select, text
...
from libs.database.sql.types import MISSING, ExecutableOptions, OrderingPair, SQLAlchemyModelT, StatementTypeT
from libs.database.sql.utils import get_instrumented_attr
...
class BaseSQLAlchemyRepository(Generic[SQLAlchemyModelT]):
    ...
    async def _find_exists(
        ...
        **filter_kwargs: dict[str, Any] | Iterable[tuple[Any, Any]],
        ...
        """
        # Build query with filters but select only 1 to check existence
        statement = self._build_query_statement(
            *filters,
        ...
        # Use EXISTS pattern with LIMIT 1 for performance
        statement = statement.with_only_columns(sql_func.count(text("1"))).limit(1)
